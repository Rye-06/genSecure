...
def main():
    ...
    video = VideoWriter('tempOutside.avi', VideoWriter_fourcc(*'MP42'), 25.0, (640, 480)) #opens the output video file stream
    ...
    while cap.isOpened():       
        ...
        if ev.type == pygame.QUIT: #checks for window close button click
            video.release()
            os.remove("tempOutside.avi") #deletes the temporary recording file
        ...
        if ev.type == pygame.MOUSEBUTTONUP: #detects for a mouse press
             ...
             if pos[0] > surfaceSize-350 and pos[0] < surfaceSize-20 and pos[1] > surfaceSize-100 and pos[1] < surfaceSize-40: #checks if the record button is clicked
                ...
                if length < 30:
                    ffmpeg_extract_subclip("tempOutside.avi", 0, length, targetname="recordingOutside" + str(curRec) + ".avi") #save the initial feed as a recording (since its length is less than 30 seconds)
                else:
                    ffmpeg_extract_subclip("tempOutside.avi", (length-30), length, targetname="recordingOutside" + str(curRec) + ".avi") #save the last thirty seconds of the feed as a recording
                clip.close()
                video = VideoWriter('tempOutside.avi', VideoWriter_fourcc(*'MP42'), 25.0, (640, 480)) #opens the output video file stream again
